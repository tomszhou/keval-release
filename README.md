...
👉 **[查看每日下载量与趋势](https://tomszhou.github.io/keval-release/)**

统计页面每小时自动更新，显示今日、昨日、累计及各版本下载量。每日数据从统计启用之日起开始记录。
...
### v1.0.3（2026-07-16）
- **局域网同步更可靠**：可手动填对方 IP 直连配对（发现不到设备时兜底），「设备与同步」显示本机 IP
- **支持"复制金库到新机器"**：新增「重置本机同步身份」；复制过来的金库重置身份后即可与原机相互识别、直接同步，无需清空重建
- 修复：主机端也会拒绝与自己配对

### v1.0.2（2026-07-16）
- **新增 TOTP 动态验证码**：网站 / 邮箱条目可保存 2FA 密钥，详情页直接显示每 30 秒滚动的 6 位验证码（带倒计时、一键复制）
- 密码等敏感字段输入框新增「显示 / 隐藏」眼睛开关，方便核对
- 兼容 `otpauth://` 链接导入；复制出的验证码 / 密钥不含空格
